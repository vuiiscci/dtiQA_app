...
public enum ModelIndex {
    ...
    NLDT_POS_HESS("nldt_pos_hess", 3, 1),
    NLDT("nldt", 4, 1), 
    NLDT_HESS("nldt_hess", 5, 1),
    NLDT_MINPACK("nldt_minpack", 999, 1), 
    LDT_WTD("ldt_wtd", 7, 1), 
    ADC("adc", -1, 0),
    RESTORE("restore", -2, 1),
    ...
    DTDT("dtdt", 70, 2), // unconstrained two tensor
     ...
     */
    ModelIndex(String invName, int numericalIndex, int ndts) {
	name = invName;
     ...
     * <p>
     * Note: this method should not be used for a <code>MultiTensorInversion</code>; see 
     * <code>getClassifiedModelIndices</code>.
     ...
     * Strings are in the array, a single index is returned per string. 
     *
     */
    public static ModelIndex[] getIndices(String[] codeStrings) {


	ModelIndex[] codes = null;
	...
	codes = new ModelIndex[codeStrings.length];
     ...
     * These indices are used to create a <code>MultiTensorInversion</code>. 
     * 
     * @param codeStrings 
     *
     */
    public static ModelIndex[][] getClassifiedModelIndices(String[] codeStrings) {

	
	ModelIndex[][] codes = new ModelIndex[codeStrings.length][];

	for (int i = 0; i < codeStrings.length; i++) {
	    codes[i] = getIndices(new String[] {codeStrings[i]});

	    // If a text code is used for a two-tensor inversion, use first code 
	    // as single-tensor initializer
	    //
	    // If a number was used, it would contain the single tensor code as well
	    // and then codes[i].length == 2
	    if (codes[i][0].numDTs > 1 && codes[i].length == 1) {
		codes[i] = new ModelIndex[] {codes[i][0], codes[0][0]};
	    }
	}
