 ...
 * <dt>Purpose:
 * 
 * <dd>Fits the tensorstick and dot model using one run of a Levenburg
 * Marquardt.
 * 
 * <dt>Description:
 * 
 ...
 * optimizing cos^{-1} sqrt(f). Perpendicular diffusivity1 is constrained to be
 * lower than the parallel diffusivity by optimising sin^(-1)sqrt(dperp/dparal)
 * and perpendicular diffusivity is constrained to be lower than the
 * perpendicular diffusivity1 by optimising sin^(-1)sqrt(dperp1/dperp2).
 * 
 * </dl>
 * 
 ...
 * 
 */
public class TensorStickDotLM_Fitter extends CompartmentFitter {
	 ...
	 */
	public TensorStickDotLM_Fitter() {
	}
	 ...
	 */
	public TensorStickDotLM_Fitter(DW_Scheme scheme) {
	...
	protected void makeCodec() {
		cod = new Codec() {
			...
			public double[] optToModel(double[] optParams) {
				...
				modelParams[10] = modelParams[10] * modelParams[4];// diff perp
																	// constrained
				modelParams[11] = Math.sin(optParams[7])
						* Math.sin(optParams[7]);// diff perp constrained
				modelParams[11] = modelParams[11] * modelParams[10];// diff
																	// perp2
	...
	public static void main(String[] args) {
		...
		TensorStickDotLM_Fitter inv = new TensorStickDotLM_Fitter(
